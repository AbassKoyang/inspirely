...
import { useCreatComment, useCreateReply, useLikeComment } from '@/lib/mutations';
...
const CommentCard = ({comment}:{comment: CommentType}) => {
    ...
    const handleCreateReply = (data: {commentId: string; comment: {content: string; parent_id?: number}}) => {
    createReply(data, {
        onSuccess: async (createdReply) => {
                queryClient.setQueryData([`replies-${data.commentId}`], (old: any) => {
                    if(!old) return
                    return {
                    ...old,
                    pages: old.pages.map((page: any, i: number) =>
                      i === 0
                        ? { ...page, results: [createdReply, ...page.results] }
                        : page
                    ),
                }})
            setreplyContent('')
    ...
    <div className='w-full mb-3 border-b last:border-0 border-gray-100 py-6 first:pt-0'>
        ...
        <div className="w-full flex items-center gap-5">
            ...
            {comment.reply_count > 0 && (
                 <HoverCard openDelay={700} closeDelay={100}>
                    <HoverCardTrigger asChild>
                        <button onClick={() => setShowReplies(!showReplies)} className="flex items-center gap-2 cursor-pointer group">
            ...
            <div className="w-full px-5 mt-3 border-l-3 border-gray-100/90">
                <div className="w-full p-4 rounded-sm bg-gray-100/90">
                    ...
                    <div className={`w-full ${isTextAreaOpen ? 'flex' : 'hidden'} items-center gap-2 justify-end mt-3`}>
                        ...
                        <Button disabled={isPending} onClick={() => handleCreateReply({commentId: String(comment.id), comment:{content: replyContent, parent_id: comment.id}})} variant='default' className='rounded-4xl cursor-pointer text-xs px-3 py-1'>Respond</Button>
                    </div>
                </div>
            </div>
        )}
