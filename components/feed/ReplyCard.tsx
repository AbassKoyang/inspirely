...
import { useLikeReply } from '@/lib/mutations';


const ReplyCard = ({comment}:{comment: CommentType}) => {
    const {mutate: likeReply} = useLikeReply(String(comment.parent_id))
  ...
  return (
    <div className='w-full mb-3 border-b last:border-0 border-gray-100 py-6 first:pt-0'>
        ...
        <div className="w-full flex items-center gap-5">
            <HoverCard openDelay={700} closeDelay={100}>
                <HoverCardTrigger asChild>
                <button onClick={() => handleLikeReply(String(comment.id))} className="flex items-center gap-2 cursor-pointer group">
